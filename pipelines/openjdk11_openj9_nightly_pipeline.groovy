...
buildMaps['zLinux'] = [test:['openjdktest'], ArchOSs:'s390x_linux']
buildMaps['ppc64le'] = [test:['openjdktest'], ArchOSs:'ppc64le_linux']
...
for ( int i = 0; i < buildPlatforms.size(); i++ ) {
	...
	jobs[platform] = {
		...
		def checksumJob
		stage('build') {
			buildJob = build job: "openjdk11_openj9_build_${archOS}",
		...
		if (buildMaps[platform].test) {
			stage('test') {
				buildMaps[platform].test.each {
					build job:"openjdk11_j9_${it}_${archOS}",
							propagate: false,
							parameters: [string(name: 'UPSTREAM_JOB_NUMBER', value: "${buildJobNum}"),
									string(name: 'UPSTREAM_JOB_NAME', value: "openjdk11_openj9_build_${archOS}")]
				}
			}
		}
		stage('checksums') {
			checksumJob = build job: 'openjdk11_openj9_build_checksum',
							parameters: [string(name: 'UPSTREAM_JOB_NUMBER', value: "${buildJobNum}"),
									string(name: 'UPSTREAM_JOB_NAME', value: "openjdk11_openj9_build_${archOS}")]
		}
		stage('publish nightly') {
			build job: 'openjdk_release_tool',
						parameters: [string(name: 'REPO', value: 'nightly'),
									...
									string(name: 'VERSION', value: 'jdk11-openj9'),
									string(name: 'CHECKSUM_JOB_NAME', value: "openjdk11_openj9_build_checksum"),
